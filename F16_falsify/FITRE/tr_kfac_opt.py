    ...
    def _prepare_model(self):
        for module in self.model.modules():
            ...
            if classname in self.known_modules:
                assert ((classname in ['Linear', 'Conv2d']) and module.bias is not None), \
    ...
    def step(self, closure=None):
        ...
        for i, m in enumerate(self.modules):
            ...
            if self.steps % self.Tf == 0:
                ...
                if p.is_cuda:
                    self.aa_inv[m] = torch.inverse(
                        self.m_aa[m] + la * torch.eye(self.m_aa[m].shape[0]).type(torch.cuda.DoubleTensor))
                    self.gg_inv[m] = torch.inverse(
                        self.m_gg[m] + la * torch.eye(self.m_gg[m].shape[0]).type(torch.cuda.DoubleTensor))
                else:
                    self.aa_inv[m] = torch.inverse(
                        self.m_aa[m] + la * torch.eye(self.m_aa[m].shape[0]).type(torch.FloatTensor))
                    self.gg_inv[m] = torch.inverse(
                        self.m_gg[m] + la * torch.eye(self.m_gg[m].shape[0]).type(torch.FloatTensor))
        ...
        vs, self.q_model_change = self.hessian_clip(grads_fun, grads_data, list(self.model.parameters()), self.kl_clip,
                                                    vs, self.weight_decay)
        if self.check_grad != 0:
            gs, q_model_change_grad = self.hessian_clip(grads_fun, grads_data, list(self.model.parameters()),
                                                        self.kl_clip, grads_data, self.weight_decay)
